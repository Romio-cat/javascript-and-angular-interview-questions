...
5. ### Hoisting
***Hoisting*** (*поднятие*, *всплытие*) - это механизм в JS, в котором объявления переменных (объявленных через `var`) и объявления функций (т.е *function declaration*) передвигаются вверх по своей области видимости. Таким образом, мы можем вызывать функции, объявленные через *function declaration* до их объявления. И обращаться к переменным, объявленным через `var` до их объявления.
```javascript
...
```
```javascript
console.log(a);//undefined, всплыло только объявление, а значение у просто объявленной переменной по умолчанию undefined
...
Основное различие между ними в том, что функции, объявленные как *Function Declaration*, создаются интерпретатором до выполнения кода (связано с всплытием), а функции, объявленные при помощи Function Expression, создвются во время выполнения выражения. Поэтому их можно вызывать до объяления функции:
```javascript
...
Существует 2 + 1 способа создать объект:
...
```
8. ### this
1) Значение this в глобальном контексте зависит от того, используется ли `use strict` или нет. В `use strict` значение `this === undefined`, в нестрогом - `this === window` (это в браузерах).
2) `this` внутри функции ссылается на внешний контекст:
```javascript
function myFunc() {
  'use strict'
  return this;
...
```
3) В контексте объекта (и в методах) `this` ссылается на этот объект:
```javascript
let obj = {
...
var objProperty = "'this' in obj.objMethod() here is not referred to the global context";//var здесь и далее используется для наглядности
obj.objMethod();//propertyValue
```
4) Используя методы call() и apply(), можно вызывать функцию в нужном контексте:
```javascript
function myFunc() {
...
myFunc.call(obj);//3
```
5) При помощи bind() можно создать новую функцию на основе имеющейся и привязать ей нужный контекст:
```javascript
function a() {
  return this.b;
}
...
func();//objb
```
6) У стрелочных функций нет своего `this`. Значение берётся снаружи:
```javascript
let obj = {
  name: 'obj',
  letters: ['a', 'b', 'c'],
  ...
  showLetters() {
    this.letters.forEach(item => alert(`${this.name}: ${item})`);
  }
}

obj.showLeters();//obj: a, obj: b, obj: c
```
7) Если использовать функцию как обработчик события, то `this` присваивается элементу, с которого начинается событие
8) В инлайновом обработчике, `this` указывает на DOM элемент, в котором расположен код события.
...
11. ### Разница между методами массива slice и splice
   - Метод `slice` имеет синтаксис `arr.slice([start], [end])` и предназначен для **копирования элементов из массива `arr`**. Возвращает массив скопированных элементов (начиная со `start` и заканчивая `end` не включая). **Копирование** подразумевает, что **исходный массив не изменяется**:
   ```javascript
   let arr = [1, 2, 3, 4, 5];
...
Различия `var` и `let` в том, что:
   ...
   ```
   - Объявление (но не присваивание!) `var` производится в начале исполнения функции или скрипта. Иными словами, объявление `var` всплывает. `let` же, в свою очередь, объявляюся и присваиваюся ровно там, где написаны.  
   Например, c `var`:
   ```javascript
   function myFunc() {
     alert(test);
   ...
   alert(window.globalVar);//6
   ```
   Но:
   ```javascript
   let globalVar = 6;

   alert(window.globalVar);//undefined, такого свойства нет.
   ```
15. ### Что такое временная мертвая зона
***Мертвая временная зона*** - это период между входом в область действия и объявлением `let`, когда к ним нет доступа. Мёртвая врменення зона заканчивается, когда `let` объявляется, а не назначается. `let` тоже поднимается, однако, именно из-за мертвой временной зоны, попытка получения доступа до назначения заканчивается ReferenceError.   
16. ### Классы в es6
...
console.log(Animal === Animal.prototype.sleep);//true
...
- Мы не можем вызвать функцию, созданную при помощи класса, (а `typeof Animal` в первом примере выше будет именно функцией) без использования `new`.
...
alert(myFirstMeal);//soup
alert(mySecondMeal);//pasta
alert(myBeverage);//tea
```
И если посложнее:
```javascript
let menu = {
...
- сделать функцию-обёртку:
```javascript
let obj = {
  objProperty: 'propertyValue',
  objMethod() {
    return `This is a ${this.objProperty}`;
  }
}

setTimeout(function(){
  obj.objMethod();//Будет работать корректно
}, 2000)
```
- использовать bind:
```javacript
...
setTimeout(obj.objMethod.bind(obj), 2000)//тоже сработает корректно
```
...
***Прототипное наследование*** - механизм в программировании, с помощью которого можно, за неимением в объекте собственного свойства или метода, найти в объекте, который для этого объекта является прототипом. Если и в этом объекте этого свойства или метода нет, то найти в объекте, который для этого объекта является прототипом, и т.д
...
- Родитель объявляется через функцию конструктор. У родителя есть приватные, публичные и защищённые свойства.
- Для реализации наследования конструктор потомка вызывает родителя в своём контексте через `apply`. После этого может добавить свои переменные и методы.
...
Через obj.\_\_proto__ мы можем получить объект, который является прототипом для этого объекта. В каждом объекте есть скрытое поле [[Prototype]], в котором хранится ссылка на прототип. Поскольку это поле скрыто и мы не может его получить напрямую, то используем obj.\_\_proto__.  
 ...
 console.log(koala.__proto__.__proto__ === Object.prototype)//true
 ```
