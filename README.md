...
```
Если бы `let` действительно не поднимались, то мы бы получили в результате 20. А из-за *временной мёртвой зоны* мы просто не можем обратиться к поднятой переменной, которую мы объявили в блоке `if`. 
